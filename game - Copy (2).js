...
carImg.onerror = () => console.error("Failed to load car2 image");
...
// Preload images and handle errors (commented out for this test)
function loadAssets() {
  return new Promise((resolve, reject) => {
    ...
    images.forEach(img => {
      ...
      img.onload = () => {
        loaded++;
        console.log(`Loaded image: ${img.src}, Total loaded: ${loaded}/${images.length}`);
...
function gameLoop() {
  ...
  obstacles.forEach(obstacle => {
    // Ensure cars stay strictly within road lanes (120 to 470)
...
document.addEventListener("keydown", function(e) {
  ...
  switch (e.key) {
    case "ArrowUp":
      ...
      // Prevent frog from leaving left or right edges
    ...
    case "ArrowLeft":
      frog.x -= step;
      // Prevent frog from leaving left edge
      if (frog.x < 0) frog.x = 0;
      console.log("Frog moved left to x:", frog.x, "y:", frog.y);
      break;
    case "ArrowRight":
      frog.x += step;
      // Prevent frog from leaving right edge
      if (frog.x > canvas.width - frog.width) frog.x = canvas.width - frog.width;
